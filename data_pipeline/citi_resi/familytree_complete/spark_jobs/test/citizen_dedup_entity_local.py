...
"""
Citizens Deduplication and Entity Resolution - V4 UPDATED
Handles ALL 20 Family Book Scenarios with cleaned dataset
"""

# ── CONFIG ─────────────────────────────────────────────────────────────────
INPUT_PATH   = "./new_data_sc_map/person_master_enriched_v5.csv"
...
def main():
    spark = SparkSession.builder \
        .appName("citizens_dedup_entity_builder_v5") \
        .config("spark.sql.legacy.timeParserPolicy", "LEGACY") \
        .getOrCreate()

    print("=" * 80)
    print("CITIZENS DEDUPLICATION - V4 (CLEANED DATASET)")
    print("=" * 80)
    ...
    print(f"✓ Loaded {person_master.count():,} records from {INPUT_PATH}")

    # Normalize fields for matching
    normalized = person_master \
        .withColumn("name_ar_norm", F.lower(F.trim(F.col("spm_full_aname")))) \
        .withColumn("name_en_norm", F.lower(F.trim(F.col("spm_full_ename")))) \
        .withColumn("dob_norm", F.to_date(F.col("spm_dob"), "M/d/yyyy")) \
        .withColumn("national_id_norm",
            F.regexp_replace(F.col("spm_national_id"), r"[\s\-]+", "")) \
        .withColumn("gender_norm", F.upper(F.trim(F.col("spm_gender").cast("string"))))
    ...
    rule2_key = F.when(
        (F.col("name_en_norm").isNotNull()) &
        (F.col("dob_norm").isNotNull()) &
        (F.col("gender_norm").isNotNull()),
        F.concat_ws("|", F.lit("NAME"), F.col("name_en_norm"),
                    F.col("dob_norm"), F.col("gender_norm"))
    )
    ...
    value_cols = [
        "spm_full_aname", "spm_full_ename", "spm_gender", "spm_dob",
        "spm_national_id", "nat_code_curr_nat", "emi_code", "cit_code"
    ]
    ...
    person_entity = best_records.select(
        ...
        F.col("cit_code").alias("city_code"),
        F.col("is_emirati"),
        F.col("naturalized_flag"),
        F.coalesce(F.col("spm_modified_date"), F.col("spm_created_date")).alias("updated_at"),
        F.col("completeness"),
        F.lit("citizen").alias("person_type")
    )

    # Create person_alias table
    person_alias = entities.select(
        F.col("entity_person_id"),
        F.col("spm_person_no"),
        F.col("cluster_key"),
        F.when(F.col("cluster_key").startswith("NID|"), 1.0)
         .when(F.col("cluster_key").startswith("NAME|"), 0.9)
         .otherwise(0.8).alias("confidence"),
        F.lit("person_master_v5_csv").alias("source")
    ).dropDuplicates(["spm_person_no"])
    ...
    print("\n" + "=" * 80)
    print(f"✓ DEDUPLICATION COMPLETE - V4")
    print(f"  Total persons: {person_entity.count():,}")
    print(f"  Emirati citizens: {person_entity.filter(F.col('is_emirati') == 'Y').count():,}")
    print(f"  Foreign spouses: {person_entity.filter(F.col('is_emirati') == 'N').count():,}")
    print("=" * 80)
